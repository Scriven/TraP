...
my $proteins = (defined $cgi->param('proteins'))?$cgi->param('proteins'):''; #'3385949,26711867,10364754';
$proteins =~ s/[^\d,]//g;
...
my $draw_disorder = (defined $cgi->param('disorder'))?$cgi->param('disorder'):undef;
...
my $record_height = 85; #how much spacing for a single protein record
...
sub seqid_to_protein {
	...
	foreach my $seq (@$seqs) {
		my $result = $dbh->selectall_arrayref("select protein from protein where genome = ? and seqid = ?;", undef, $genome, $seq);
		foreach my $protein (@$result) {
			$proteins{$protein->[0]}++;
		}
	}
...
sub get_details {
	...
	foreach my $protein (split /,/, $proteins) {
		...
		#Get all the disordered hits from my temporary oates table, to be changed if/when this is moved to SUPERFAMILY
		if ($draw_disorder) {
			my $disordered_hits = $dbh->selectall_arrayref("SELECT start, end, predictor FROM $disorder_db.ass_disorder WHERE protein = ? ORDER BY start ASC", undef, $protein);
...
<svg xmlns="http://www.w3.org/2000/svg"
	 ...
	 hit_quality = svg_document.getElementById('hit_quality');
	  ...
	  var title_text = hit.getElementsByTagName('title').item(0);
	  if (! title_text ) {
	     return false;
	  }
...
sub draw_disorder {
	...
	foreach my $disorder (@{$protein_details{$protein}{'disorder'}}) {
		...
		my $y = $middle;
		$y += ($predictor eq "VLXT")?-($disorder_height+1):1;
		my $width = $end - $start;

		$result .= <<EOF
	<rect x="$start" y="$y" width="$width" height="$disorder_height" style="fill: url(#disorder); stroke:#000; stroke-width:0.5" onmouseover="show_tip(evt)" onmouseout="hide_tip(evt)">
		<title>$predictor Predicted Disorder</title>
...
sub draw_protein {
	...
	foreach my $seqid (@$names) {
		push @names, '<a xlink:href="http://supfam.org/SUPERFAMILY/cgi-bin/gene.cgi?genome='.$seqid->[0].';seqid='.$seqid->[1].'">'.$seqid->[1].'</a>';
	}
	my $name = join ', ', @names;
...
sub sf_key {
	...
	$result = "<text x=\"0\" y=\"$dy\" font-size=\"12\">Key:</text>";
	$dy += 5;
	$text_dy = $dy + 8;

	$result .= "<rect x=\"5\" y=\"$dy\" width=\"10\" height=\"10\" fill=\"#ccc\" ry=\"2\" rx=\"2\" />";
	$result .= "<text x=\"17\" y=\"$text_dy\" font-size=\"10\">Predicted Structure</text>";
	$dy+=12;
	$text_dy = $dy + 8;
	...
	if ($is_weak_hits) {
		my $y = $dy + 10;
		$result .= "<line x1=\"10\" y1=\"$dy\" x2=\"10\" y2=\"$y\" style=\"fill:none;stroke-dasharray:1,1;stroke:#000;stroke-width:1\"/>";
		$result .= "<text x=\"17\" y=\"$text_dy\" font-size=\"10\">Weak Structural Hit</text>";
		$dy+=12;
		$text_dy = $dy + 8;	
	}
	...
	if ($draw_disorder) {
		$result .= "<rect x=\"5\" y=\"$dy\" width=\"10\" height=\"10\" fill=\"url(#disorder)\" stroke=\"black\" stroke-width=\"0.5\" />";
		$result .= "<text x=\"17\" y=\"$text_dy\" font-size=\"10\">Predicted Disorder</text>";
		$dy+=12;
		$text_dy = $dy + 8;
	}
	...
	$result .= "<text x=\"150\" y=\"$dy\" font-size=\"12\">Superfamilies:</text>";
	$dy += 5;
	$text_dy = $dy + 8;
	...
	foreach my $sf (sort {$sf_details{$a}{'label'} <=> $sf_details{$b}{'label'}} keys %sf_details) {
		...
		$result .= "<rect x=\"155\" y=\"$dy\" width=\"10\" height=\"10\" fill=\"$colour\" stroke=\"black\" stroke-width=\"0.5\" />";
		$result .= "<text x=\"167\" y=\"$text_dy\" font-size=\"10\">$key</text>";
		$dy+=12;
		$text_dy = $dy + 8;
	}
